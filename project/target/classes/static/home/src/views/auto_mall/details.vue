<template>
	<div class="diy_details page_auto_mall" id="auto_mall_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_auto_mall'>
							<div_auto_mall :obj="obj"></div_auto_mall>
...
<script>
	import div_auto_mall from "@/components/diy/div_auto_mall.vue";
		...
		methods:{
			async get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.auto_mall_id;
							}
			},
						/**
			 * 初始化收藏状态
				...
				if (this.state_collect) {
					...
					this.$get('~/api/collect/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/auto_mall/set?auto_mall_id=' + _this.obj["auto_mall_id"], {
								collect
							}, (res) => {
								if(res.result){

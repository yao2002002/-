      ...
      <el-row class="rows row2">
         ...
         <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
                ...
                <el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
                                                      <el-button v-if="$check_action('/delivery_arrangements/table','add') || $check_action('/delivery_arrangements/view','add')" @click="$router.push('./view?')" class="add">添加</el-button>
            <el-button v-if="$check_action('/delivery_arrangements/table','del') || $check_action('/delivery_arrangements/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
                        </el-col>
	    ...
	    <el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
	                <el-table-column fixed type="selection" tooltip-effect="dark" width="55">
                      ...
                      {{ get_user_registered_user(scope.row['registered_user']) }}
                                              ...
                                              <el-table-column prop="delivery_time" @sort-change="$sortChange" label="交付时间"                                v-if="$check_field('get','delivery_time')" min-width="200">
                                      <template slot-scope="scope">
                      {{ $toTime(scope.row["delivery_time"],"yyyy-MM-dd hh:mm:ss") }}
                    </template>
                                </el-table-column>
                                              <el-table-column prop="delivery_location" @sort-change="$sortChange" label="交付地点"                                v-if="$check_field('get','delivery_location')" min-width="200">
                                </el-table-column>
                                              <el-table-column prop="delivery_arrangements" @sort-change="$sortChange" label="交付安排"                                v-if="$check_field('get','delivery_arrangements')" min-width="200">
                                </el-table-column>
      ...
      <el-table-column sortable prop="update_time" label="更新时间" min-width="200">
        <template slot-scope="scope">
          {{ $toTime(scope.row["update_time"],"yyyy-MM-dd hh:mm:ss") }}
                ...
                <el-table-column fixed="right" label="操作" min-width="120" v-if="$check_action('/delivery_arrangements/table','set') || $check_action('/delivery_arrangements/view','set') || $check_action('/delivery_arrangements/view','get')
					 ">
        

      <template slot-scope="scope">
        <router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
                     v-if="$check_action('/delivery_arrangements/table','set') || $check_action('/delivery_arrangements/view','set') || $check_action('/delivery_arrangements/view','get')"
  ...
  export default {
    ...
    data() {
      return {
        ...
        showModal: false,
        // 获取数据地址
        url_get_list: "~/api/delivery_arrangements/get_list?like=0",
        url_del: "~/api/delivery_arrangements/del?",

	...
	<el-main class="bg edit_wrap comtable_e">
		...
		<el-row class="row_ce"> 
					...
					<div v-else-if="$check_field('get','user_name')">{{form['user_name']}}</div>
							...
							<el-form-item prop="complaint_title">
					<div slot="label">
						<i style="color: red;">*</i>
						投诉标题
					</div>
					...
					<div v-else-if="$check_field('get','complaint_time')">{{form['complaint_time']}}</div>
							...
							<el-form-item prop="complaint_content">
					<div slot="label">
						<i style="color: red;">*</i>
						投诉内容
					</div>
	...
	export default {
			...
			return {
				...
				form: {
								"registered_user": 0, // 注册用户
										"user_name":  '', // 用户姓名
										"complaint_title": null, // 投诉标题
										"complaint_time":  '', // 投诉时间
										"complaint_content": null, // 投诉内容
									"examine_state": "未审核",
							"examine_reply": "",
		...
		methods: {
			...
			get_obj_after(json, func){
																				if(this.form["complaint_time"]=="0000-00-00 00:00:00"){
				  this.form["complaint_time"] = null;
				}
				if(parseInt(this.form["complaint_time"]) > 9999){
					this.form["complaint_time"] = this.$toTime(parseInt(this.form["complaint_time"]),"yyyy-MM-dd hh:mm:ss")
				}
